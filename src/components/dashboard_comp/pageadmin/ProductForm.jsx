...
const ProductForm = forwardRef(({ product, onSuccess, isNewProduct, createProduct }, ref) => {
...
const resetForm = () => {
  ...
  // Exposez la méthode `resetForm` via la référence
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNewProduct) {
      ...
      try {
        ...
        onSuccess(); // Réussite de la mise à jour
      } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="  mb-10 bg-white dark:bg-slate-500 shadow-md rounded-md p-4 space-y-4">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <label className="text-sm font-medium">Lien d&apos;affiliation</label>
          <input
            type="text"
