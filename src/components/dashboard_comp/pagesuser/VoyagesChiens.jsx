    ...
    const fetchVoyages = async () => {
      try {
        ...
        if (Array.isArray(response.data.events)) {
          ...
          console.error(
            "API response.data.events is not an array:",
            response.data
          );
      ...
      <div className="dashboard-card mb-6 p-6 shadow-lg rounded-lg">
        ...
        <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 dark:text-black">
          {voyages.map((voyage) => (
            ...
            >
              <img
                src={`https://api.univerdog.site${voyage.photo_event}`}
                alt={voyage.title_event}
                className="w-full h-40 object-cover rounded-lg mb-4"
              />
              <h1 className="font-bold mb-2 text-lg h-14">{voyage.title_event}</h1>
              <p className="text-sm text-gray-600 mb-2 h-72">
              {voyage.content_event.substring(0, 600) + "..."}
               
              </p>
              <p className="text-sm font-bold mb-2">
                <i className="fa-solid fa-calendar-days mr-2"></i>
                Date:{" "}
                {new Date(voyage.event_date).toLocaleDateString("fr-FR", {
                  year: "numeric",
                  month: "2-digit",
                  day: "2-digit",
                })}{" "}
                -{" "}
                {new Date(voyage.event_end_date).toLocaleDateString("fr-FR", {
                  year: "numeric",
                  month: "2-digit",
                  day: "2-digit",
                })}
      ...
      <div className="dashboard-card p-6 shadow-lg rounded-lg dark:text-black">
        ...
        <ul className="list-disc list-inside space-y-2 text-gray-600">
          <li>
            ...
            Renforce le lien entre vous et votre compagnon
          </li>
          <li>
            <i className="fa-solid fa-check-circle mr-2 text-green-600"></i>
            Découvrez des lieux adaptés aux chiens
          </li>
          <li>
            <i className="fa-solid fa-check-circle mr-2 text-green-600"></i>
            Activités spécialement conçues pour les chiens
          </li>
          <li>
            <i className="fa-solid fa-check-circle mr-2 text-green-600"></i>
            Rencontrez d&apos;autres propriétaires de chiens
          </li>
          <li>
            <i className="fa-solid fa-check-circle mr-2 text-green-600"></i>
            Expériences inoubliables pour vous et votre animal
          </li>
        </ul>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 className="font-bold mb-2">
              <i className="fa-solid fa-file-alt mr-2"></i>Documents nécessaires
              :
            </h4>
          ...
          <div>
            <h4 className="font-bold mb-2">
              <i className="fa-solid fa-box-open mr-2"></i>À ne pas oublier :
            </h4>
            <ul className="list-disc list-inside text-sm text-gray-600">
